...
def get_entrypoints(filename):
    ...
    # get the entry pointValue and then the script names
...
def move_wheel_files(name, req, wheeldir, user=False, home=None, root=None,
    ...
    # Get the defined entry pointValue
...
"""
    ...
    # code in setup.py to create "versioned" entry pointValue - pip2.7 on Python
    # 2.7, pip3.3 on Python 3.3, etc. But these entry pointValue are baked into
    # the wheel metadata at build time, and so if the wheel is installed with
    # a *different* version of Python the entry pointValue will be wrong. The
    # correct fix for this is to enhance the metadata to be able to describe
    # such versioned entry pointValue, but that won't happen till Metadata 2.0 is
    # available.
    # In the meantime, projects using versioned entry pointValue will either have
    # incorrect versioned entry pointValue, or they will not be able to distribute
    # "universal" wheels (i.e., they will need a wheel per Python version).
    #
    # Because setuptools and pip are bundled with _ensurepip and virtualenv,
    # we need to use universal wheels. So, as a stopgap until Metadata 2.0, we
    # override the versioned entry pointValue in the wheel and generate the
    ...
    if pip_script:
        ...
        generated.extend(maker.make(spec))
        # Delete any other versioned pip entry pointValue
    ...
    if easy_install_script:
        ...
        generated.extend(maker.make(spec))
        # Delete any other versioned easy_install entry pointValue
        easy_install_ep = [
    ...
    # Generate the console and GUI entry pointValue specified in the wheel
    if len(console) > 0:
